...
public class DiscResourceIntTests {
    ...
    public void testFindById() {
...

        {
            final JSONObject response =
                    doJsonGet(String.format("/api/v1.0/%s", disc.getId()));
    ...
    private JSONObject doJsonGet(final String pPath) {
        try (
                ...
                Response response = client.target(UriBuilder.fromUri(uri).path(pPath))
    ...
    private JSONObject doJsonPost(final String pPath, final JSONObject pData) {
        try (
                Client client = ClientBuilder.newClient();
                Response response = client.target(UriBuilder.fromUri(uri).path(pPath))
                        .request(MediaType.APPLICATION_JSON).post(Entity.json(pData.toString()))) {
            return new JSONObject(response.readEntity(String.class));
        }
    }
    ...
    private JSONObject doBytesPost(final String pPath, final String pData) {
        try (
                Client client = ClientBuilder.newClient();
                Response response = client.target(UriBuilder.fromUri(uri).path(pPath))
                        .request(MediaType.APPLICATION_JSON).post(Entity.entity(pData.getBytes(), MediaType.APPLICATION_OCTET_STREAM))) {
            return new JSONObject(response.readEntity(String.class));
        }
    }
}
